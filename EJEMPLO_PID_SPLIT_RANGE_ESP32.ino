/*
  SimuPLC Lab 1.7.0-split-range-v10 - ESP32 desde FBD
  Placa objetivo: ESP32 DevKit V1 / WROOM-32
  Entradas digitales: INPUT_PULLUP (LOW = contacto cerrado a GND).
  Entradas analogicas: ADC de 12 bits (0..4095).
*/
...
const uint8_t PIN_PWM1 = 25;
const uint8_t PIN_PWM2 = 26;
const uint8_t digitalInputPins[SAFE_DIGITAL_INPUTS] = { 13 };
const uint8_t analogInputPins[SAFE_ANALOG_INPUTS] = { 36, 39 };
const uint8_t outputPins[SAFE_OUTPUTS] = { 23 };
const uint8_t pwmOutputPins[SAFE_PWM_OUTPUTS] = { 25, 26 };
const uint8_t analogOutputPins[SAFE_ANALOG_OUTPUTS] = { 25 };
const bool outputActiveLow[SAFE_OUTPUTS] = { false };
...
const char* const pwmOutputTags[SAFE_PWM_OUTPUTS] = { "PWM1", "PWM2" };
...
enum NodeType : uint8_t {
  ...
  T_AI=40,T_SCALE=41,T_GT=42,T_LT=43,T_EQ=44,T_GTE=45,T_LTE=46,T_HYST=47,T_PWM=48,T_AO=49,T_CONST=50,T_PID=51,T_SPLIT=52
};

const uint8_t nodeType[SAFE_NODES] = { 40, 40, 51, 52, 48, 48 };
const uint8_t nodePins[SAFE_NODES] = { 0, 0, 2, 1, 1, 1 };
const uint8_t nodeDigitalInputIndex[SAFE_NODES] = { 255, 255, 255, 255, 255, 255 };
const uint8_t nodeAnalogInputIndex[SAFE_NODES] = { 0, 1, 255, 255, 255, 255 };
const uint8_t nodeOutputIndex[SAFE_NODES] = { 255, 255, 255, 255, 255, 255 };
const uint8_t nodePWMOutputIndex[SAFE_NODES] = { 255, 255, 255, 255, 0, 1 };
...
const float nodeP2[SAFE_NODES] = { 4095.0f, 4095.0f, 0.1f, 100.0f, 100.0f, 100.0f };
const float nodeP3[SAFE_NODES] = { 0.0f, 0.0f, 0.05f, 50.0f, 1000.0f, 1000.0f };
const float nodeP4[SAFE_NODES] = { 100.0f, 100.0f, 0.0f, 2.0f, 8.0f, 8.0f };
const float nodeP5[SAFE_NODES] = { 0.0f, 0.0f, 100.0f, 100.0f, 0.0f, 0.0f };
...
const uint8_t nodeClamp[SAFE_NODES] = { 1, 1, 0, 1, 1, 1 };

const uint16_t connSrc[SAFE_CONNS] = { 0, 1, 2, 3, 3 };
const uint8_t connSrcPin[SAFE_CONNS] = { 0, 0, 0, 0, 1 }; // 0=salida principal/LLENAR, 1=VACIAR
const uint16_t connDst[SAFE_CONNS] = { 2, 2, 3, 4, 5 };
const uint8_t connPin[SAFE_CONNS] = { 0, 0, 0, 0, 0 };
const uint8_t connInv[SAFE_CONNS] = { 0, 0, 0, 0, 0 };
const uint8_t connSignal[SAFE_CONNS] = { 1, 1, 1, 1, 1 }; // 0=digital,1=analogica

bool bValue[SAFE_NODES] = {false};
float aValue[SAFE_NODES] = {0};
float splitFill[SAFE_NODES] = {0};
float splitDrain[SAFE_NODES] = {0};
...
float analogSourceValue(uint16_t source,uint8_t sourcePin){if(source>=N_NODES)return NAN;if(nodeType[source]==T_SPLIT)return sourcePin==1?splitDrain[source]:splitFill[source];return aValue[source];}
float readAnalogPin(uint16_t node,uint8_t pin){
  for(uint16_t k=0;k<N_CONNS;k++) if(connDst[k]==node && connPin[k]==pin && connSignal[k]==1) return analogSourceValue(connSrc[k],connSrcPin[k]);
  return NAN;
}
