...
// ======================================================
// ====== AUTO LIBRARY LOADER FROM GITHUB (EVC) =========
// ======================================================

async function getFilesRecursive(owner, repo, path){
  ...
  for(const item of data){
    ...
    if(item.type === "dir"){
      const sub = await getFilesRecursive(owner,repo,item.path);
...
async function loadLibrary(){
  ...
  const repoOwner = "pasir2025";
  const repoName = "RODAJESD";
  const basePath = "EVC";

  try{

    const files = await getFilesRecursive(repoOwner, repoName, basePath);

    for(const file of files){

      if(!file.toLowerCase().endsWith(".evc")) continue;
      ...
      const fakeFile = new File([blob], file);

      await loadEvcFile(fakeFile,false);

    }

    renderLibrary();
    setStatus("Biblioteca cargada automáticamente desde GitHub");

  }
  catch(err){

    console.error(err);
    setStatus("Error cargando biblioteca");
