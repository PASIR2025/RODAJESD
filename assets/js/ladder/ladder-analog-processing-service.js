(function (global) {
  ...
  const VERSION = '1.7.1-phase3-pid';
  ...
  const PROCESSOR_TYPES = [TYPES.CONST, TYPES.SCALE, TYPES.GT, TYPES.LT, TYPES.EQ, TYPES.GTE, TYPES.LTE, TYPES.HYST, TYPES.PID, TYPES.PWM, TYPES.AO];
  ...
  function normalizeProcessor(element) {
    ...
    element.type = type;
    element.signalType = (isConstant(type) || isScale(type)) ? 'analog' : (isOutputSink(type) ? 'analog-output' : 'mixed');
  ...
  function drawProcessor(element, x, y) {
    ...
    const width = scalePx(element.type === TYPES.PID ? 154 : (element.type === TYPES.SCALE ? 138 : (constant ? 136 : (sink ? 142 : 122))));
    ...
    if (!sink) {
      ctx.textAlign = 'right'; ctx.fillStyle = isScale(element) ? '#087e9a' : (outputOn ? '#d10000' : '#555');
      ctx.fillText((isScale(element) || constant || isPID(element)) ? 'A' : 'Q', left + width - scalePx(7), y + scalePx(2));
    }
    ctx.restore();
  }
  ...
  function createEditFields() {
    ...
    wrap.innerHTML = `
      ...
      <label data-field="voltageMin">Voltaje mínimo AO<input class="edit-input" id="ladderAnalogVoltageMin" type="number" step="any"></label>
      <label data-field="voltageMax">Voltaje máximo AO<input class="edit-input" id="ladderAnalogVoltageMax" type="number" step="any"></label>
  ...
  function captureEditFields() {
    ...
    }else if(isScale(element)||isPID(element)||isOutputSink(element)){
  ...
  function buildAnalogTopology(elements) {
    ...
    (elements || []).forEach(function (element) {
      ...
      if (type === TYPES.INPUT || type === TYPES.CONST) {
        ...
        if (isScale(element)||isPID(element)) addEndpoint(pinId(element, 'out'), { elementId: element.id, elementType: type, kind: 'output', side: 'out', role: 'A' });
      }
    });
  ...
  function applyToScan(scan, runtime) {
    ...
    Object.keys(runtime.elements || {}).forEach(function (elementId) {
      ...
      if(isScale(element)||isPID(element)||isOutputSink(element)){
        scan.elements[elementId]={active:Number.isFinite(info.outputValue),energized:Number.isFinite(info.outputValue),inPower:Number.isFinite(info.inputValue),analogInput:info.inputValue,analogOutput:info.outputValue,physicalOutput:isOutputSink(element)};
        if(element.label&&Number.isFinite(info.outputValue))scan.analog[element.label]=info.outputValue;
      }else{
  ...
  function appendProcessorVariables() {
    ...
    processorElements().forEach(function (element) {
      ...
      else if (isAO(element)) value.textContent = formatNumber(runtime.outputValue, element.decimals, 'V');
      ...
      else if (isAO(element)) detail.textContent = 'Entrada: ' + formatNumber(runtime.inputValue, element.decimals, element.unit) + ' · DAC ' + element.voltageMin + '…' + element.voltageMax + ' V';
  ...
  function patchFunctions() {
    ...
    try {
      ...
      window.addFreePinsForElement = addFreePinsForElement = function (element) {
        ...
        if (!isOutputSink(element)) addProPin({
