(function(global){
  ...
  function render(options){
    ...
    nodes.forEach(function(node){
      ...
      if(node.type==='input' || node.type==='output' || node.type==='M'){
        ...
        if(node.output) node.output.classList.toggle('signal-on',active);
      }
      if(isAnalogSource && node.output) node.output.classList.toggle('signal-on',running);
    ...
    if(running){
      connections.forEach(function(connection){
        ...
        var source=nodes.find(function(node){ return node && node.output===connection.from; });
        applySignal(connection,source ? source.value : 0);
      });
    }else{
      updateConnections();
    }
  ...
  function reset(options){
    ...
    nodes.forEach(function(node){
      ...
      if(node.output) node.output.classList.remove('signal-on');
